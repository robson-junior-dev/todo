...
public class ProjectRepositoryWithBagRelationshipsImpl implements ProjectRepositoryWithBagRelationships {
    ...
    public Optional<Project> fetchBagRelationships(Optional<Project> project) {
        return project.map(this::fetchCollaborators);
    }
    ...
    public List<Project> fetchBagRelationships(List<Project> projects) {
        return Optional.of(projects).map(this::fetchCollaborators).orElse(Collections.emptyList());
    }

    Project fetchCollaborators(Project result) {
        return entityManager
            .createQuery(
                "select project from Project project left join fetch project.collaborators where project is :project",
                Project.class
            )
    ...
    List<Project> fetchCollaborators(List<Project> projects) {
        ...
        List<Project> result = entityManager
            .createQuery(
                "select distinct project from Project project left join fetch project.collaborators where project in :projects",
                Project.class
            )
